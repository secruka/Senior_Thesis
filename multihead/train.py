...
train.py
========
3-stage training script for the multi-head clock recognition model.

Stages:
  1. Backbone Warmup  - 144-class time classification with CrossEntropyLoss
  2. Component Training - DeepProbLog component(X,R,H,M) head initialisation
  3. Time Integration  - DeepProbLog time(X,Hour,Minute) with constraints

Usage:
  python multihead/train.py --data_root ../../clock_kaggle --run all
  python multihead/train.py --data_root ../../clock_kaggle --run warmup
  python multihead/train.py --data_root ../../clock_kaggle --run heads
  python multihead/train.py --data_root ../../clock_kaggle --run time
"""
...
def collate_fn(batch):
    """Custom collate for ClockDataset (img, ClockLabels)."""
...
def train_warmup(args, device):
    ...
    train_ds = ClockDataset(
        args.data_root, args.annotations, subset="train",
        transform=get_transform(train=True), max_samples=args.max_samples,
    )
    valid_ds = ClockDataset(
        args.data_root, args.annotations, subset="valid",
...
# ---------------------------------------------------------------------------
# Stage 2: Component Training (DeepProbLog)
# ---------------------------------------------------------------------------

def train_heads(args, device, shared_net: MultiHeadClockNet):
    """Train 3 heads with DeepProbLog component(X, R, H, M)."""
    print("=" * 60)
    print("STAGE 2: Component Training (DeepProbLog)")
    print("=" * 60)
    ...
    # Shared optimizer with lower LR for backbone, higher for heads
    ...
    # Datasets
    train_torch = ClockDataset(
        args.data_root, args.annotations, subset="train",
        transform=get_transform(train=True), max_samples=args.max_samples,
    )
    valid_torch = ClockDataset(
        args.data_root, args.annotations, subset="valid",
        transform=get_transform(train=False), max_samples=args.max_samples,
    )

    train_dpb = DPBComponentDataset(train_torch)
    valid_dpb = DPBComponentDataset(valid_torch)
    ...
    # Save weights
    os.makedirs(args.save_dir, exist_ok=True)
    path = os.path.join(args.save_dir, "after_heads_shared.pth")
    torch.save(shared_net.state_dict(), path)
    print(f"Saved after-heads weights to {path}")

    return shared_net, model


# ---------------------------------------------------------------------------
# Stage 3: Time Integration (DeepProbLog)
# ---------------------------------------------------------------------------

def train_time(args, device, shared_net: MultiHeadClockNet):
    """Fine-tune with DeepProbLog time(X, Hour, Minute) constraints."""
    print("=" * 60)
    print("STAGE 3: Time Integration (DeepProbLog)")
    print("=" * 60)
    ...
    fine_lr = args.lr / 10
    optimizer = torch.optim.Adam(shared_net.parameters(), lr=fine_lr)
    ...
    train_dpb = DPBTimeDataset(train_torch)
    valid_dpb = DPBTimeDataset(valid_torch)
    ...
    # Save final weights
    os.makedirs(args.save_dir, exist_ok=True)
    path = os.path.join(args.save_dir, "final_shared.pth")
    torch.save(shared_net.state_dict(), path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-Head Clock Recognition - 3-Stage Training"
    )
    parser.add_argument("--data_root", type=str, required=True,
                        help="Path to clock_kaggle directory")
    parser.add_argument("--annotations", type=str, default=None,
                        help="Path to annotations.csv (default: <data_root>/annotations.csv)")
    parser.add_argument("--prolog", type=str, default=None,
                        help="Path to ProbLog file (default: models/clock_multihead.pl)")
    parser.add_argument("--run", type=str, default="all",
                        choices=["warmup", "heads", "time", "all"],
                        help="Which stage(s) to run")
    parser.add_argument("--epochs_warmup", type=int, default=10)
    parser.add_argument("--epochs_heads", type=int, default=5)
    parser.add_argument("--epochs_time", type=int, default=10)
    parser.add_argument("--batch_size", type=int, default=8,
                        help="Batch size for Stage 1 warmup")
    parser.add_argument("--batch_size_dpb", type=int, default=4,
                        help="Batch size for DeepProbLog stages 2&3")
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--save_dir", type=str, default="weights_multihead",
    ...
    parser.add_argument("--load_heads", type=str, default=None,
                        help="Path to pre-trained heads weights")
    ...
    if args.prolog is None:
        # Look for the .pl file relative to this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        args.prolog = os.path.join(script_dir, "..", "models", "clock_multihead.pl")

    # Validate
    if not os.path.exists(args.annotations):
        print(f"ERROR: annotations.csv not found at {args.annotations}")
        print("Run extract_keypoints.py first to generate annotations.")
        sys.exit(1)
    ...
    elif args.load_heads is not None:
        shared_net.load_state_dict(torch.load(args.load_heads, map_location=device))
        print(f"Loaded heads weights from {args.load_heads}")

    if args.run in ("heads", "all"):
        shared_net, _ = train_heads(args, device, shared_net)

    # ---------------------------------------------------------------
    # Stage 3: Time Integration
    # ---------------------------------------------------------------
    if args.run in ("time", "all"):
        if args.run == "time" and args.load_heads is not None:
            shared_net.load_state_dict(
                torch.load(args.load_heads, map_location=device)
            )
            print(f"Loaded heads weights from {args.load_heads}")
        shared_net, _ = train_time(args, device, shared_net)
