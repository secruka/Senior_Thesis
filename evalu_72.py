...
def angle_cls_n(hand_x: float, hand_y: float, cx: float, cy: float, n_bins: int) -> int:
    """angle_cls_12 の一般化（n_bins=12/72 など）。

    - 12時方向を cls=0
    - 時計回りに cls が増える
    - 最近傍量子化: step = 360/n_bins, オフセットは step/2
    """
    dx = float(hand_x) - float(cx)
    dy = float(hand_y) - float(cy)
    ...
    cls = int(np.floor((angle + step / 2.0) / step)) % int(n_bins)
    return cls
...
class RotationCsvDataset(Dataset):
    def __init__(
        ...
        self.minute_bins = int(minute_bins)
    ...
    def __getitem__(self, idx):
        ...
        if self.task == "dial":
            ...
            h_cls = angle_cls_n(hx, hy, cx, cy, self.hour_bins)
            m_cls = angle_cls_n(mx, my, cx, cy, self.minute_bins)
...
def infer_num_classes_from_ckpt(state_dict: dict, default: int) -> int:
    """ResNetClassifier の最終Linearの出力次元をチェックポイントから推定する。

    checkpoint に含まれる fc 層の weight/bias の shape[0] を見て返す。
    """
    possible_keys = (
...
def main():
    ...
    if args.hour_pth and args.minute_pth:
        hour_sd = torch.load(args.hour_pth, map_location=device)
        minute_sd = torch.load(args.minute_pth, map_location=device)

        hour_bins = infer_num_classes_from_ckpt(hour_sd, default=12)
        minute_bins = infer_num_classes_from_ckpt(minute_sd, default=12)

        hands_ds = RotationCsvDataset(
            args.data_root,
            csv_path,
            args.split,
            task="hands",
        ...
        hour_model = ResNetClassifier(num_classes=hour_bins).to(device)
        minute_model = ResNetClassifier(num_classes=minute_bins).to(device)

        hour_model.load_state_dict(hour_sd)
        minute_model.load_state_dict(minute_sd)
        h_acc, m_acc, joint = eval_hands(hour_model, minute_model, hands_ld, device)
        rand_h = 1.0 / max(hour_bins, 1)
        rand_m = 1.0 / max(minute_bins, 1)
        rand_joint = rand_h * rand_m
        print(
            f"[HANDS] split={args.split}  hour_acc={h_acc:.4f}  minute_acc={m_acc:.4f}  joint={joint:.4f} "
            f"(random: hour={rand_h:.4f}, minute={rand_m:.4f}, joint={rand_joint:.4f})"
        )
